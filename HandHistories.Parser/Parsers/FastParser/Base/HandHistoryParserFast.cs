...
{
    abstract public class HandHistoryParserFast : IHandHistoryParser
    {
        ...
        public virtual bool RequiresSittingOutAssesment
        {
            get { return false; }
        }
        ...
        public bool AmountsAsBigBlindMultiples { get; set; } = false;
        public string FileName { get; set; }
        private Limit LastLimit = null;
        ...
        public string ParseTournamentId(string handText)
        {
            try
            {
        ...
        protected abstract string ParseTournamentId(string[] handLines);
        ...
        protected GameDescriptor ParseGameDescriptor(string[] handLines)
        {
            ...
            {
                Limit limit = ParseLimit(handLines);
                LastLimit = LastLimit == null || limit.BigBlind > LastLimit.BigBlind ? limit : LastLimit;
                return new GameDescriptor(format,
                                       SiteName,
                                       ParseGameType(handLines),
                                       ParseTournamentId(handLines),
                                       LastLimit,
                                       ParseTableType(handLines),
                                       ParseSeatType(handLines)); 
            }
        ...
        /// <summary>
        /// Fixes error where actions at the end of the hand (e.g. winnings)
        /// are always treated as showdown actions. Even if play ended before the 
        /// river.
        /// </summary>
        /// <param name="actions">All actions in the hand</param>
        /// <param name="playerNum">The number of players involved in hand</param>
        /// <returns>Corrected action list</returns>
        protected List<HandAction> FixShowdownHands(List<HandAction> actions, int playerNum)
        {
            Street lastStreet = Street.Null;
            int playersLeft = playerNum;
            List<HandAction> newActions = new List<HandAction>();

            for (int i = 0; i < actions.Count; ++i)
            {
                switch (actions[i].Street)
                {
                    case Street.Showdown:
                        if (playersLeft == 1 || actions[i].HandActionType == HandActionType.UNCALLED_BET)   // Not a proper showdown
                            actions[i].Street = lastStreet;
                        break;

                    default:
                        if (actions[i].HandActionType == HandActionType.FOLD)
                            --playersLeft;
                        lastStreet = actions[i].Street;
                        break;
                }
            }
            return actions;
        ...
        protected PlayerList FindSittingOutPlayers(HandHistory hand)
        {
            string firstPlayer = hand.HandActions[0].PlayerName;
            HashSet<string> playersFound = new HashSet<string>();
            playersFound.Add(firstPlayer);
            int i = 1;

            while (!hand.HandActions[i].PlayerName.Equals(firstPlayer))
            {
                playersFound.Add(hand.HandActions[i].PlayerName);
                ++i;
            }

            foreach (Player player in hand.Players)
            {
                if (!playersFound.Contains(player.PlayerName))
