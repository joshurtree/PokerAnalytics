...
{
    public sealed class Poker888FastParserImpl : HandHistoryParserFast
    {
        static readonly CultureInfo invariant = CultureInfo.InvariantCulture;

        public override SiteName SiteName
        {
            get { return SiteName.Values.Pacific; }
        }
        ...
        protected override string ParseTournamentId(string[] handLines)
        {
            return new Regex(@"(?<=Tournament #)\d+").Match(handLines[3]).Value;
        }
        ...
        protected override TableType ParseTableType(string[] handLines)
        {
            ...
            decimal callCost = Math.Max(limit.BigBlind, Math.Max(limit.SmallBlind, limit.Ante));
            if (callCost == 0)
                return TableType.FromTableTypeDescriptions(TableTypeDescription.Any);
            ...
            foreach (Player player in playerList)
            {
                ...
                if (player.StartingStack / callCost == 5m) return TableType.FromTableTypeDescriptions(TableTypeDescription.PushFold);
                if (player.StartingStack / callCost <= 10m) playersBelow10bb++;
                if (player.StartingStack / callCost > 29m) playersAbove20bb++;
            ...
            if (tableStack / callCost / playerList.Count <= 17.5m)
            {
                if (isjackPotTable)
                {
        ...
        protected override Limit ParseLimit(string[] handLines)
        {
            ...
            return Limit.FromSmallBlindBigBlind(lowLimit, highLimit, line.Contains("$") ? Currency.USD : Currency.PlayMoney);
        }

        protected override Buyin ParseBuyin(string[] handLines)
        {
            if (handLines[3].Contains("Free"))
                return new Buyin(0, 0, Currency.USD);

            MatchCollection values = new Regex(@"\$(?<value>\d*\.?\d{2})").Matches(handLines[3]);

            decimal prizePool = Decimal.Parse(values[0].Groups["value"].Value);
            decimal rake = 0;

            if (values.Count > 1)
                rake = Decimal.Parse(values[1].Groups["value"].Value);

            return new Buyin(prizePool, rake, Currency.USD);
        }
        ...
        protected override List<HandAction> ParseHandActions(string[] handLines, GameType gameType = GameType.Unknown)
        {
            ...
            for (int i = 6; i < handLines.Length; i++)
            {
                ...
                if (handLine[0] == '*')
                {
                    ...
                    if (handLine[3] == 'S')
                    {
                        //Not all hands end in a showdown
                ...
                if (summary)
                {
                    ...
                    {                        
                        ...
                        handActions.Add(new WinningsAction(playerName, HandActionType.WINS, amount, 0, currentStreet));
                        continue;                        
                    }
        ...
        protected override PlayerList ParsePlayers(string[] handLines)
        {
            ...
            for (int i = 0; i < seatCount; i++)
            {
                ...
                int amountOffset = handLine.Contains("$") ? 3 : 2;
                decimal amount = ParseAmount(handLine.Substring(openParenIndex + amountOffset, 
                                                                handLine.Length - openParenIndex - 3 - 2));

                playerList.Add(new Player(playerName, amount, seat));
            }

            foreach (string handline in handLines)
            {
                if (handline[0] == 'D' && handline.StartsWith("Dealt"))
                {
                    //Parse hero hole cards
                    int openSquareIndex = handline.IndexOf('[');
                    HoleCards holeCards = parseHoleCards(handline);
                    string playerName = handline.Substring(9, openSquareIndex - 10);
                    playerList.First(p => p.PlayerName.Equals(playerName)).HoleCards = holeCards;
                }
            }
