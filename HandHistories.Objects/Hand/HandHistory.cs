...
namespace HandHistories.Objects.Hand
{
    public class HandHistory : HandHistorySummary
    {
        ...
        // Change amount quantities to multiple of given value (usually big blind)
        public void Normalise(decimal value)
        {
            foreach (HandAction handAction in HandActions)
            {
                handAction.Amount /= value;
            }

            foreach (Player player in Players)
            {
                player.StartingStack /= value;
            }
            Rake /= value;
            TotalPot /= value;
        }
